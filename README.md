<html lang="en">
<head>
  <title>Search Table Page</title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
  </head>
  <style>
   #header
 {
 background-color:lightyellow;
 margin:0;
 }
 .text
 {
 text-align:center;
 color:black;
 }
  </style>
  <body>
  <div class="container-fluid">
  <div  id="header">
	<div class="row"></br></br>
	<div class="col-sm-1">
	</div>
	<div class="col-sm-8">
	<div class="btn-group btn-group-lg " >
  <button type="button" class="btn btn-info active"style="background-color:green;">&nbsp;&nbsp;&nbsp;&nbsp;    All Books           &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</button>
  <button type="button" class="btn btn-info active"style="background-color:green;">&nbsp;&nbsp;&nbsp;&nbsp;    Books & E Books     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</button>
  <button type="button" class="btn btn-info active"style="background-color:green;">&nbsp;&nbsp;&nbsp;&nbsp;    Magazine & Journals &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</button>
  <button type="button" class="btn btn-info active"style="background-color:green;">&nbsp;&nbsp;&nbsp;&nbsp;    Media               &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</button>
  </div>
  </br>
  </br>
  <b><input   type="text" style="width:70%" placeholder="Search"></b>
  <a href="" class="url"><h4><u>Advance Search</u></h4></a>
</div></br></br>
<div class="col-sm-1">
<label for="ex2"></label>
    <h2><span class="label label-default" style="background-color:blue">Search</span></h2>
</div>
</div>
</div></br>

<div style="width: 100%; height: 10px; border-bottom: 1px solid black; ">
      <span style=" background-color: #696969;">
      </span>
	  </div>

<div class="row">
<div class="col-sm-1">
</div>
<div class="col-sm-2">
<label><h4><b>Search Result</b></h4></label>
</br>



<a href=""><< Previous</a>
|
<a href="">  Next >></a>
</div>
<div class="col-sm-10">
</div>

<div class="col-sm-1">
<div class="dropdown">
    <button class="btn btn-default dropdown-toggle" type="button" id="menu1" data-toggle="dropdown">100 Per page
    <span class="caret"></span></button>
    <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
      <li role="presentation"><a role="menuitem" tabindex="-1" href=""></a>200 Per Page</li>
      <li role="presentation"><a role="menuitem" tabindex="-1" href=""></a>300 Per page </li>
	  <li role="presentation"><a role="menuitem" tabindex="-1" href=""></a>400 Per page </li>
	  </ul>
	  </div>
	  </div>




	  <div class="col-sm-1">
	  <div class="dropdown">
    <button class="btn btn-default dropdown-toggle" type="button" id="menu1" data-toggle="dropdown">Sort
    <span class="caret"></span></button>
    <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
      <li role="presentation"><a role="menuitem" tabindex="-1" href=""></a>Name</li>
      <li role="presentation"><a role="menuitem" tabindex="-1" href=""></a>Author</li>
	  <li role="presentation"><a role="menuitem" tabindex="-1" href=""></a>Cost</li>
	  </ul>
	  </div>
	  </div>

</div>



</div> 
 <div style="width: 99%; height: 10px; border-bottom: 1px solid black; text-align: center">
      <span style=" background-color: #696969;">
      </span>
	  </div>
	   <div class="page-header" id="header">
	  <div class="row">
	  <div class="col-sm-4">
</div>
	  <div class="col-sm-6">
<label for="ex2"><h4><b>Description</b></h4></label>
	 </div>
	 <div class="col-sm-2">
<label for="ex2"><h4><b>Select</b></h4></label>
	 </div>
	 </div>
	 </div>
	  <div class="row"> 
<div class="col-sm-4">
</br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

 <img src="https://media.wiley.com/product_data/coverImage300/30/04706591/0470659130.jpg" alt="Los Angeles" style="width:25%;"></br>
 </br>
 
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;


 <img src="data:image/jpeg;base64," alt="Los Angeles" style="width:25%;"></br></br>
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 <img src="data:image/jpeg;base64," alt="Los Angeles" style="width:25%;">
</div>
<div class="col-sm-6">
</br>
<label for="ex2"><h4><b> 1.<i> Handbook of Agricultural Entomology </i>by Helmut van Emden.</br>
                               Language:English</br>
                               Publisher:New York:[Time-Life Books],[1969-1970]</b></h4></label></br></br></br></br></br>
							   
<label for="ex2"><h4><b> 2. <i>Wild about books </i>by Judy Sierra;Marc Tolon Brown</br>
                               Language:English</br>
                               Publisher:New York:Alfred A.Knopf,2004.</b></h4></label></br></br></br></br></br></br>
							   
 <label for="ex2"><h4><b>3. <i>The purchase of books </i>by public libraries by Great Britain</br>
                               Language:English</br>
                               Publisher:London,H.M.Stationary Off.,1972</b></h4></label></br></br>
		
							   
</div>
<div class="col-sm-2">
</br>
<div class="checkbox">
   <label for="ex2"><input type="checkbox"></label>	
		</div></br></br></br></br></br></br></br>
		<div class="checkbox">
   <label for="ex2"><input type="checkbox"></label>	
		</div></br></br></br></br></br></br>
		<div class="checkbox">
   <label for="ex2"><input type="checkbox" checked="checked"></label>	
		</div></br></br></br>
		<a href="RegisterPageDisplay.do" class="url"><h4><u>Read|Download</u></h4></a>
		</div>
		</div>
		<div class="row">
		<div class="col-sm-1">
		</div>
		<div class="col-sm-8">
		</div>
		<div class="col-sm-1">
    <h2><span class="label label-default"style="background-color:red;">Cancel</h2>  
		</div>
		<div class="col-sm-2">
		<h2><span class="label label-default" style="background-color:blue">Reserve</h2>
		</div>
		</div>
		<div class="row">
		<div class="col-sm-1">
		</div>
		<div class="col-sm-11">
		<label for="ex2"><h4><b>Speaker Notes</b></h4></label></br>	
</div>
</div>
<div class="row">
<div class="col-sm-1">
</div>
<div class="col-sm-1">
<ul class="pager">
<li><a href="#">Previous</a></li>
</ul>
</div>
<div class="col-sm-1">
<img src="https://3.bp.blogspot.com/-nWFQOMITbsQ/TaUlwWMqRtI/AAAAAAAAAfA/uXiFqh69Wzw/s80/sunrise.png" alt="Los Angeles" style="width:100%;">
</div>
<div class="col-sm-2">
<b><i> "Handbook of Agricultural Entomology" </i>by Helmut van Emden is a landmark publication for students</b></br>
</div>
<div class="col-sm-3">
<img src="http://digital-storytime.com//graphics/DS_Logo.png" alt="Los Angeles" style="width:35%;"><b>This whimsical storyline is one that kids will love and identify with too, since young children are often as hard on their books as any monkey in the zoo!</b></br></br>
</div>
<div class="col-sm-3">
<img src="https://www.scribendi.com/images/cms/thumbnails/How-to-Write-a-Report-on-a-Book_100x100.jpg" alt="Los Angeles" style="width:20%;"><b>"Historical fiction is mostly smoke and mirrors"</b></br></br>
</div>
<div class="col-sm-1">
<ul class="pager">
<li><a href="#">Next</a></li>
</ul>
		</div>
		</div>
		</div>
</body>
</html>
